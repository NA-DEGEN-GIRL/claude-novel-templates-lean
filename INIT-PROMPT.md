...
### 1.5단계: 전문가 컨셉 평가
시나리오 3개를 **story-consultant 에이전트**(`claude-novel-templates-lean/.claude/agents/story-consultant.md`)로 평가한다. 4개 관점(연재 편집자, 장르 전문가, 캐릭터/감정 편집자, 구조 엔지니어)에서 각 시나리오를 채점하고 리스크를 진단한다.
...
### 3.5단계: 플롯 사전 검증 (/why-check plan)
생성된 `plot/master-outline.md`와 `plot/arc-01.md`에 대해 **why-checker의 Planning Mode**를 실행한다.
...
검증 완료 후 모순 0건이면 5단계로. **만약 4단계에서 plot 파일(master-outline, arc-01)이 수정되었다면, 3.5단계 /why-check plan을 해당 파일에 대해 1회 재실행한다.**
...
3. story-consultant 에이전트(`claude-novel-templates-lean/.claude/agents/story-consultant.md`)로 컨셉 평가를 수행해줘 (4개 관점: 연재 편집자, 장르 전문가, 캐릭터/감정, 구조). GO가 나오면 다음으로.
4. 내가 OK하면 claude-novel-templates-lean/의 구조대로 전체 파일을 생성해줘.
   ...
   - config.json 등록
5. 파일 생성 후, plot/master-outline.md와 plot/arc-01.md에 대해 `/why-check plan` 실행 (why-checker Planning Mode). PLANNING GAP 발견 시 즉시 수정.
...
프롬프트 1의 3~5단계 절차 전체를 수행하고, /why-check plan + 교차 검증도 수행해.
모순 발견 시 즉시 수정.
